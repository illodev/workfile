...
- [x] README and example teach the guarded form, with the one-line why
- [x] A test loads a config using the guarded form from a directory where the package
...
- 2026-08-05 12:02Z illodev@local#2cddaf94 — Verified 2026-08-05 under T-0174. #1: `packages/search-local/README.md` teaches the guarded form with the reason in the comment, and the 2026-07-30 21:18Z note records Fube's guarded config loading on a clean git-archive clone with its CI job green.

#2 was the one thing genuinely missing — the criterion asks for a test and none existed. Written now: `the guarded import the README teaches loads where the package is absent` in integrations.test.ts carries the README snippet verbatim, loads it from a temp directory where `@illodev/workfile-search-local` does not resolve, and asserts `integrations === []`. The premise was confirmed separately: the unguarded form from the same directory fails with ERR_MODULE_NOT_FOUND, so the empty array is a result and not an accident of the package being present.
