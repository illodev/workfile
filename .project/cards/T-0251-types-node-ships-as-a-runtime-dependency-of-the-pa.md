...
Measured in a consumer on 2026-09-11, moving `@illodev/workfile` 0.10.0 → 0.13.0 in Fube (`pnpm add -D -w`): the lockfile diff re-keyed **79 entries** whose peer-dependency chains carry `@types/node` — `next`, `sharp`, `react-dnd`, the Storybook and Vite plugins — from 26.1.2 to 26.5.0, none of them anything to do with this package. The consumer's own `@types/node` ranges (`^22`, `^24`, `^25`) did not ask for 26.x; this package did. `scripts/audit-consumer.ts` resolves exactly this `dependencies` map and its header says why it matters, and `docs/ui.md` calls zero runtime dependencies a published guarantee for the UI — the CLI's runtime imports nothing from `@types/node` either, because nothing can: it is types.
...
- [ ] A consumer that moves to the patch sees only the package's own lockfile entry change, measured with `git diff --stat pnpm-lock.yaml` in Fube
