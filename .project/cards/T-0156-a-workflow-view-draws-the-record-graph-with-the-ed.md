...
- [x] Pan and zoom, with connectors that read at both extremes
...
- 2026-08-04 21:56Z illodev@local#cfe281b4 — Crash reported from the browser and fixed: 'Cannot read properties of null (reading x)'. The pan handler read drag.current!.x inside the setView updater, which React runs after the handler returns — and pointerup lands in between on a plain click, so the ref was already null. The guard above it was true at event time and said nothing about update time, and the non-null assertion is what let that compile: it asserted over the one moment the value can be absent. A single click on the background reproduced it. Fixed by taking the origin as an argument, so there is no later moment to read it at, and the same assertion shape was removed from the three byId.get(focus)! reads. Pan and zoom are now pure functions in workflow.ts with tests: zooming is asserted to hold the point under the cursor across five factors rather than merely changing scale, and to clamp at both ends. That is criterion 2's first half, which had been left unchecked precisely because nothing tested it. Also on this pass, per the request: the drawer opens inside the view. onOpen navigated to Explorer, Docs or Memory depending on the id shape, which for a canvas is the opposite of the point — the reason to draw the graph was to stop going card by card. It now opens the shared RecordDrawer in place, reading through a new kind-agnostic api.record so a doc renders the same as a card, and onOpen survives on an 'Open in its view' control so leaving is a choice. Verified through the server: T-0156 card, DOC-0002 doc at 71594 chars, ADR-0010 memory and CHG-0111 change all answer 200 with bodies.
- 2026-08-04 22:44Z illodev@local#cfe281b4 — Verified on the platform: PR #18 green on all nine jobs — ubuntu 22/24 (49s, 56s), macos 22/24 (1m22s, 1m18s), windows 22/24 (2m09s, 1m57s), plus smoke, codeql and doctor. Merged at 996d6b3. Verified in the browser by the reporter across five rounds, which is what found everything the tests could not: the pan crash, the opening jump, the type size, the dot that would not select, the reload that landed on a different view, and the doc link that closed the sheet without a word. The view ships at 9.7 kB, 3.9 gzipped, and dist/ui went from 900K to 912K — the ADR-0010 bet, that a hand-written layout would cost less than a library for a graph this size, held.

## Activity

- 2026-08-04 21:33Z illodev@local#cfe281b4 · claimed
- 2026-08-04 21:49Z illodev@local#cfe281b4 · doing → review
- 2026-08-04 22:44Z illodev@local#cfe281b4 · review → done
